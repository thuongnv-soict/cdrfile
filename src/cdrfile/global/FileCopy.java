...
public class FileCopy {
	...
	public static void main(String[] args) {
		...
		{
			System.err
					.println("Usage: java FileCopy <source file> <destination>");
		} else {
			// Call copy() to do the copy, and display any error messages it
			// throws.
			try {
				copy(args[0], args[1]);
			} catch (IOException e) {
				System.err.println(Global.Format(new java.util.Date(),
						"dd/MM/yyyy HH:mm:ss")
						+ " : - ERROR in module FileCopy : " + e.getMessage());
			}
		}
	}
	...
	public static void copy(String from_name, String to_name)
		...
		if (to_file.exists()) {
			...
			System.out
					.print("Overwrite existing file " + to_name + "? (Y/N): ");
			System.out.flush();
		...
		try {
			...
			while ((bytes_read = from.read(buffer)) != -1) // Read bytes until
			// EOF
			{
