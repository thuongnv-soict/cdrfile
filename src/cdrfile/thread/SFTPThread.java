...
public class SFTPThread extends ThreadInfo {
    ...
    protected void processSession() throws Exception
    {
        ...
        try
        {
            ...
            {	
            	checkFtpTime = true;
				if (rs.getString("ftp_begin_time") != null && rs.getString("ftp_begin_time") != null)
				{
					...
					if (cp1 == 1 || cp2 == 2) {
						checkFtpTime = false;
						writeLogFile("Skip connect to host " + rs.getString("NOTE") + "=>" + rs.getString("FTP_HOST_IP"));
					}
				}
    ...
    protected int DoListingRemote(java.sql.Connection pConnection, Vector pVectorParam, FtpParam plistParam, ChannelSftp channelSftp, int pTimeOut, int pFtpID, String pReceiveFrom, String pDirReceive, String pFileInfo, String pFileName, long pFileSize, String pReceiveHeader, String pReceiveExt, int pRemoteFileSplitByDay, String pCurrentDir, String extToGetFile, int extSeqFrom, int extSeqTo) throws Exception
    {
                        ...
                        if (pRemoteFileSplitByDay == 1)
                        {
                            if (Global.isNumeric(contain.getFilename()))
                            {
                                if ((pCurrentDir != null) && ((Integer.parseInt(pCurrentDir)) <= (Integer.parseInt(contain.getFilename()))))
                                {
                                    ...
                                    if (DoListingRemote(pConnection, pVectorParam, plistParam, channelSftp, pTimeOut, pFtpID, mReceiveFrom, pDirReceive, pFileInfo, pFileName, pFileSize, pReceiveHeader, pReceiveExt, pRemoteFileSplitByDay, contain.getFilename(), extToGetFile, extSeqFrom, extSeqTo) != 0)
                                    {
                                        return ( -1);
                                    }
                                }
        ...
        int pRenameAfterDownload, int pSeqFrom, int pSeqTo, int pRenameType, String pNewPreFix, String pNewExt, String pMailTo, int pSeqAfterDownload, int pMinSeq, int pMaxSeq, int pCurrSeq, int pWaitingLastFile, String extToGetFile, long fileSize, int extSeqFrom, int extSeqTo, int checkSizeForgetFile) throws Exception
    {
        try
        {
            ...
            {
                if (DoListingRemote(pConnection, pVectorParam, plistParam, channelSftp, pTimeOut, pFtpID, pReceiveFrom, pDirReceive, pFileInfo, pFileName, pFileSize, pReceiveHeader, pReceiveExt, pRemoteFileSplitByDay, pCurrentDir, extToGetFile, extSeqFrom, extSeqTo) == 0)
                {
                    if (vListDownload.size() > 0)
                    {
                        DoDownload(pConnection, pVectorParam, plistParam, channelSftp, pFtpID, pDirReceive, pRemoteFileSplitByDay, pLocalFileSplitByDay, pZipAfterDownload, pRenameAfterDownload, pSeqFrom, pSeqTo, pRenameType, pNewPreFix, pNewExt, pSeqAfterDownload, pMinSeq, pMaxSeq, pCurrSeq, pWaitingLastFile, pFileSize, pMailTo, fileSize, checkSizeForgetFile);
                    }
                }
                else
                {
                ...
                if (Global.ExecuteOutParameterInt(pConnection, "SELECT count(*) INTO ? " + "FROM sys_param a,sys_param_detail b " + "WHERE a.id = b.ptr_id and " + "b.ptr_name='AutoDownloadMissedFile' " + "AND ptr_value='TRUE'") > 0)
                {
                    ...
                    DoLoadMissingFile(pConnection, channelSftp, pFtpID, pReceiveFrom, pDirReceive, pRemoteFileSplitByDay, pZipAfterDownload, pRenameAfterDownload, pSeqFrom, pSeqTo, pRenameType, pNewPreFix, pNewExt, "");
            ...
            if ((pSendTo != "") && (pSendTo != null))
            {
                ...
                DoUpload(pConnection, channelSftp, pFtpID, pDirSend, pSendTo, pLocalFileSplitByDay, pCurrentDir, pRemoteFileSplitByDay, pFileInfo, pFileName, pFileSize);
    ...
    protected int DoDownload(java.sql.Connection pConnection, Vector pVectorParam, FtpParam plistParam, ChannelSftp channelSftp, int pFtpID, String pDirReceive, int pRemoteFileSplitByDay, int pFileSplitByDay, int pZipAfterDownload, int pRenameAfterDownload, int pSeqFrom, int pSeqTo, int pRenameType, String pNewPreFix, String pNewExt, int pSeqAfterDownload, int pMinSeq, int pMaxSeq, int pCurrSeq, int pWaitingLastFile, long pFileSizeDownloaded, String pMailTo, long pFileSizeForDownload, int checkSizeForGetFile) throws Exception
    {
...
*/
    ...
    protected int DoUpload(java.sql.Connection pConnection, ChannelSftp channelSftp, int pFtpID, String pDirSend, String pSendTo, int pLocalFileSplitByDay, String pCurrentDir, int pRemoteSplitFileByDay, String pFileInfo, String pFileName, long pFileSize) throws Exception
    {
                        ...
                        if (pLocalFileSplitByDay == 1)
                        {
                            if (Global.isNumeric(pFile.getName()))
                            {
                                if ((pCurrentDir != null) && ((Integer.parseInt(pCurrentDir)) <= (Integer.parseInt(pFile.getName()))))
                                {
                                    ...
                                    if (DoUpload(pConnection, channelSftp, pFtpID, mDirSend, pSendTo, pLocalFileSplitByDay, pFile.getName(), pRemoteSplitFileByDay, pFileInfo, pFileName, pFileSize)!= 0)
                                        return -1;
                                }
                            }
                        }
