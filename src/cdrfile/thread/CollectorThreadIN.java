...
public class CollectorThreadIN extends ThreadInfo {
		...
		try {
			while (rs.next() && miThreadCommand != THREAD_STOP) {
				...
				if (rs.getInt("local_split_file_by_day") == 1)
					collec.CollectiveIN(mConnection, rs.getInt("id"), IOUtil
							.FillPath(rs.getString("collect_dir"),
									Global.mSeparate)
							+ rs.getString("current_dir"), rs
							.getString("file_name"));
				else
					collec.CollectiveIN(mConnection, rs.getInt("id"), rs
			...
			if (cdrfileParam.OnErrorResumeNext.compareTo("TRUE") == 0) {
				mSQL = "UPDATE import_header SET status="
						+ Global.StateCollectTrafficTurnoverError + ",note='"
						+ e.toString() + "' WHERE file_id = " + pFileID;
				Global.ExecuteSQL(mConnection, mSQL);
				writeLogFile("      .Exported with error :" + e.toString());
				mConnection.commit();
			} else {
				throw e;
			}
